...
<template>
  <form text-center justify-center items-center max-w-150 py6 flex="~ col gap-3" @submit.prevent="oauth">
    ...
    <div text-secondary text-sm max-w-100 mb-2>
      {{ $t('user.keycloak_sign_in_desc') || "Connectez-vous avec votre compte Keycloak pour accéder à l'application." }}
    </div>
    
    <!-- Affichage des informations de connexion -->
    <div v-if="keycloakServer" text-sm text-secondary mb-2 p-2 bg-gray:5 rounded>
      <div><strong>Serveur :</strong> {{ keycloakServer }}</div>
      <div><strong>Realm :</strong> {{ keycloakRealm }}</div>
    </div>
    
    <!-- Message d'erreur -->
    <div v-if="displayError" role="alert" p-2 m-0 text-sm text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/20 rounded>
    ...
    <!-- Lien d'aide -->
    <div v-if="keycloakServer" text-secondary text-sm flex mt-4>
